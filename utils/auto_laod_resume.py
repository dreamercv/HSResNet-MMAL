...
def auto_load_resume(model, path, status):
    if status == 'train':
        ...
        nums_epoch = [int(name.replace('air_hs_epoch', '').replace('.pth', ''))
                      for name in pth_files if '.pth' in name]
        if len(nums_epoch) == 0:
            return 0, init_lr
        else:
            max_epoch = max(nums_epoch)
            pth_path = os.path.join(
                path, 'air_hs_epoch' + str(max_epoch) + '.pth')
        ...
        for k, v in checkpoint['model_state_dict'].items():
            ...
            new_state_dict[name] = v
        model.load_state_dict(new_state_dict)
        epoch = checkpoint['epoch']  # 128
        # print(f"11111111111111  {checkpoint}")
