    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        if (epoch % save_interval == 0) or (epoch == end_epoch):
            ...
            torch.save({
                ...
                'learning_rate': lr,
            }, os.path.join(save_path, 'air_hs_epoch' + str(epoch) + '.pth'))
        ...
        if len(checkpoint_list) == max_checkpoint_num + 1:
            idx_list = [int(name.replace('air_hs_epoch', '').replace('.pth', '')) for name in checkpoint_list]
            min_idx = min(idx_list)
            os.remove(os.path.join(save_path, 'air_hs_epoch' + str(min_idx) + '.pth'))
