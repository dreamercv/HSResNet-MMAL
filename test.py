...
if set == 'CUB':
    ...
    root = '/home/data/FGVC/fgvc_variant'  # dataset path
    # model path
    pth_path = "./checkpoint/aircraft_hs_avg/air_hs_epoch64.pth"  # 64 drop0.5 88.51%
    num_classes = 100
...
with torch.no_grad():
    for i, data in enumerate(tqdm(testloader)):
        ...
        print(local_logits.max(1, keepdim=True))
        print(pred)
        print("11111111111111111111111\n")
        # print(f"---------pred---------{pred.item()}-----------y----{y.view_as(pred).item()}-------")
