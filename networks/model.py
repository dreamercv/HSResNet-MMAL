...
def nms(scores_np, proposalN, iou_threshs, coordinates):
    ...
    indices_coordinates = np.concatenate((indices_coordinates, np.arange(
        0, windows_num).reshape(windows_num, 1)), 1)[indices]  # [339,6]
    indices_results = []
...
class APPM(nn.Module):
    def __init__(self):
        ...
        self.avgpools = [nn.AvgPool2d(ratios[i], 1)
    ...
    def forward(self, proposalN, x, ratios, window_nums_sum, N_list, iou_threshs, DEVICE='cuda'):
        ...
        all_scores = torch.cat([fm_sum[i].view(batch, -1, 1)
                               for i in range(len(ratios))], dim=1)
        windows_scores_np = all_scores.data.cpu().numpy()
        window_scores = torch.from_numpy(
            windows_scores_np).to(DEVICE).reshape(batch, -1)
        ...
        for i, scores in enumerate(windows_scores_np):
            ...
            proposalN_indices.append(np.concatenate(
                indices_results, 1))   # reverse

        proposalN_indices = np.array(
            proposalN_indices).reshape(batch, proposalN)
...
class MainNet(nn.Module):
    def __init__(self, proposalN, num_classes, channels):
        ...
        self.pretrained_model = resnet.resnet50(
            pretrained=False, pth_path=pretrain_path)
        self.rawcls_net = nn.Linear(channels, num_classes)
        print("3333333333333333331")
        print(self.rawcls_net)
        self.APPM = APPM()
    ...
    def forward(self, x, epoch, batch_idx, status='test', DEVICE='cuda'):
        ...
        local_imgs = torch.zeros([batch_size, 3, 448, 448]).to(
            DEVICE)  # [N, 3, 448, 448]
        ...
        local_fm, local_embeddings, _ = self.pretrained_model(
            local_imgs.detach())  # [N, 2048]
        ...
        if status == "train":
            # window_imgs cls
            window_imgs = torch.zeros([batch_size, self.proposalN, 3, 224, 224]).to(
                DEVICE)  # [N, 4, 3, 224, 224]
            for i in range(batch_size):
            ...
            window_imgs = window_imgs.reshape(
                batch_size * self.proposalN, 3, 224, 224)  # [N*4, 3, 224, 224]
            _, window_embeddings, _ = self.pretrained_model(
                window_imgs.detach())  # [N*4, 2048]
            proposalN_windows_logits = self.rawcls_net(
                window_embeddings)  # [N* 4, 200]
        else:
            proposalN_windows_logits = torch.zeros(
                [batch_size * self.proposalN, self.num_classes]).to(DEVICE)
