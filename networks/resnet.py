...
class HSBlock(nn.Module):
    ...
    def __init__(self, in_channels, block_groups=5):
        super(HSBlock, self).__init__()
        self.in_c = in_channels
        self.block_groups = block_groups

        self.first_c = self.in_c // self.block_groups + self.in_c % self.block_groups

        self.other_c = self.in_c // self.block_groups

        self.modulelist = nn.ModuleList()

        for block_i in range(self.block_groups - 1):
            if block_i == 0:
                one_split = nn.Sequential(
                    conv3x3(self.other_c, self.other_c * 2),
                    nn.BatchNorm2d(self.other_c * 2))
            elif block_i == self.block_groups - 2:
                one_split = nn.Sequential(
                    conv1x1(self.other_c * 2, self.other_c),
                    nn.BatchNorm2d(self.other_c))
            else:
                one_split = nn.Sequential(
                    conv3x3(self.other_c * 2, self.other_c * 2),
                    nn.BatchNorm2d(self.other_c * 2))
    ...
    def forward(self, x):
        ...
        for i in range(1, len(self.modulelist)):
            ...
            start_c = self.first_c + self.other_c * i

            x_i = torch.cat(
                [x[:, start_c:start_c + self.other_c, :, :], x_next2], dim=1)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        if use_HSBlock and stride == 1:
            self.conv2 = HSBlock(width)
        else:
