...
class STANFORD_CAR(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            normale_transform = transforms.Compose([
                transforms.Resize((self.input_size, self.input_size), Image.BILINEAR),
                transforms.RandomHorizontalFlip(),
                transforms.ColorJitter(brightness=0.2, contrast=0.2)(img),
                transforms.ToTensor(),
                transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
            ])
            img = normale_transform(img)
...
class FGVC_aircraft(data.Dataset):
    def __init__(self, input_size, root, is_train=True, data_len=None, transform=None):
        self.input_size = input_size
        self.root = root
        print("-----------------")
        print(self.root)
        ...
        self.normale_transform = transforms.Compose([
            transforms.Resize((self.input_size, self.input_size), Image.BILINEAR),
            transforms.RandomHorizontalFlip(),
            transforms.ColorJitter(brightness=0.2, contrast=0.2),
            # transforms.GaussianBlur(3),
            # transforms.RandomPerspective(),
            # transforms.RandomRotation((-90, 90), center=(self.input_size // 2, self.input_size // 2)),
        ...
        train_label_file = open(os.path.join(self.root, 'train_images.txt'), "r", encoding='utf-8')
        test_label_file = open(os.path.join(self.root, 'test_images.txt'), 'r', encoding='utf-8')

        train_img_label = []
        test_img_label = []

        for line in train_label_file.readlines():
            info = line.lstrip().rstrip().split()
            train_img_label.append([os.path.join(self.root, "train_images", info[0]), int(info[1]) - 1])
        for line in test_label_file.readlines():
            info = line.lstrip().rstrip().split()
            test_img_label.append([os.path.join(self.root, "test_images", info[0]), int(info[1]) - 1])
        self.train_img_label = train_img_label[:data_len]
        self.test_img_label = test_img_label[:data_len]
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            img = self.normale_transform(img)
